...
def build_ook_waveform(
    bits, pulse_us, zero_gap_us, one_gap_us, sync_us=0, sync_gap_us=0
):
    """
...
pulse_us = st.sidebar.slider(
    "Pulse width (HIGH)", 100, 1000, timing["pulse_us"], step=50
)
zero_gap_us = st.sidebar.slider(
    "Zero gap (LOW)", 100, 2000, timing["zero_gap_us"], step=50
)
one_gap_us = st.sidebar.slider(
    "One gap (LOW)", 500, 4000, timing["one_gap_us"], step=50
)
...
with tab1:
    ...
    **Pulse Distance** means every bit starts with the same short HIGH pulse, and the
    ...
    style_ax(
        ax1,
        title=f"Sync + first 8 data bits  [{first_8}]",
        ylabel_left="Carrier ON",
        ylabel_right="Carrier OFF",
    )

    # Annotate sync
    if sync_us > 0:
        ax1.annotate(
            "← 8 ms sync pulse (carrier on, no data) →",
            xy=(sync_us / 2, 1.15),
            color=ORANGE,
            fontsize=9,
            ha="center",
...
with tab2:
    ...
    demo_bits = "011"
    t_real, lvl_real, _, _ = build_ook_waveform(
        demo_bits, pulse_us, zero_gap_us, one_gap_us
    )
    ...
    fig2, (ax_real, ax_fm) = plt.subplots(
        2, 1, figsize=(14, 5), facecolor=DARK_BG, gridspec_kw={"hspace": 0.5}
    )
    ...
    style_ax(
        ax_real,
        title='What the remote transmits  (bits "0, 1, 1")',
        ylabel_left="Carrier ON",
        ylabel_right="Carrier OFF",
    )
    # Annotate bits
    ...
    ax_fm.set_title(
        "What you see in Audacity  (rtl_fm output)",
        color=WHITE,
        fontsize=13,
        fontweight="bold",
        pad=12,
    ...
    for b in demo_bits:
        ...
        ax_fm.text(
            pos + gap / 2,
            -1.05,
            f"{'long' if b == '1' else 'short'} noise burst = bit {b}",
            ha="center",
            color=YELLOW,
            fontsize=8,
        )
...
with tab3:
    ...
    **Your timing reference values** (from the decoded protocol):
    ...
    style_ax(
        ax3,
        title=f"Reading bits from the waveform: [{demo_pattern}]",
        ylabel_left="Carrier ON",
        ylabel_right="Carrier OFF",
    )
    ...
    short_patch = mpatches.Patch(
        color=YELLOW, alpha=0.4, label=f"Short gap ({zero_gap_us} µs) = 0"
    )
    long_patch = mpatches.Patch(
        color=RED, alpha=0.4, label=f"Long gap ({one_gap_us} µs) = 1"
    )
    ax3.legend(
        handles=[short_patch, long_patch],
        loc="upper right",
        facecolor=PANEL_BG,
        edgecolor=MUTED,
        labelcolor=WHITE,
...
with tab4:
    ...
    st.markdown(
        "Notice how the left half (address) stays the same and only the right half (command) changes:"
    )
